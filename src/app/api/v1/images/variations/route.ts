...
export async function POST(request: Request) {
  try {
    ...
    const targetUrl = `${selectedRoute.baseURL}/images/variations`;
    ...
    try {
      ...
      // Calculate cost (assuming fixed cost per image variation request, using inputTokenPrice as the rate)
      ...
      for (const [key, value] of formData.entries()) {
        if (key !== 'image') { requestBodyToLog[key] = value; }
      }
      await db.run(
