...
基础地址默认使用该供应商首选协议的 `base_url`（new-api 与 DeepSeek 会自动去掉 `/v1`、`/v1beta` 后缀），也可以单独填写“余额查询基础地址”。查询默认复用供应商凭据；需要单独凭据时填写“覆盖查询密钥”，它与自定义请求头一起加密存储，管理接口只返回是否已配置。编辑时留空会保留已保存的密钥和请求头。
